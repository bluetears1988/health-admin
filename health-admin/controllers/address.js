...
class Ctrl{
	 ...
	 */
	routes() {
		this.app.get('/api/address/location', this.getLocation.bind(this))
	...
	getLocation(req, res, next){
		...
		// let qs = {
		// 	  location: `${lat},${lng}`,
		// 	  key: 'EGVBZ-5TOR5-UZVI5-Q3ZKA-Y7C6H-NOFYS'
		// 	};

		// this.http.get({
		// 	url: 'https://apis.map.qq.com/ws/geocoder/v1/?location=' + lat + ',' + lng + '&key=' + qs.key,
		// }, (err, response, data) => {
		// 	res.send(data);
		...
		request.get({
		    ...
		    qs: {
		      location: `${lng},${lat}`,
		      key: '26edf5e492b354f97ccdfef46c4d5c45',
		    }
		  }, (err, response, data) => {
		    if (!err && response.statusCode === 200) {
		    	res.send(data);
